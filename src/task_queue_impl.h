...
static inline void ntt_task_queue_push_back_impl(ntt_task_queue_t *self,
                                                 ...
                                                 int *first) {
  mtx_lock(&self->mtx);
  ntt_list_push_back(&self->queue, &task->node, first);
  cnd_signal(&self->cnd);
  mtx_unlock(&self->mtx);
}
...
ntt_task_queue_pop_front_blocking_impl(ntt_task_queue_t *self, int *last) {
  mtx_lock(&self->mtx);
  ntt_node_t *node = ntt_list_pop_front(&self->queue, last);
  while (node == NULL) {
    cnd_wait(&self->cnd, &self->mtx);
    node = ntt_list_pop_front(&self->queue, last);
  }
  mtx_unlock(&self->mtx);
